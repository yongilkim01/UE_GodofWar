...
namespace Debug
{
	static void Print(const FString& DebugMessage, const FColor& DisplayColor = FColor::MakeRandomColor(), bool bAssert = false, int32 InKey = -1, float TimeToDisplay = 7.f )
	{
		if (nullptr != GEngine)
		{
			GEngine->AddOnScreenDebugMessage(InKey, TimeToDisplay, DisplayColor, DebugMessage);

			if (FColor::Yellow == DisplayColor)
			{
